 ...
 */
import { connectPairedTransport, PairingConnectionError, type PairedTransport } from './paired-websocket-transport'
...
export async function connectOrcad(offer: PairingOffer, options: ConnectOrcadOptions = {}): Promise<OrcadConnection> {
...
async function connectWithTimeout(offer: PairingOffer, timeoutMs: number): Promise<PairedTransport> {
  let timedOut = false
