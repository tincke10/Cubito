...
const rootOf = (handle: ReturnType<typeof createDiffHud>) => handle.root as unknown as FakeElement
const connectionLineOf = (root: FakeElement) => root.children[0]!
const modeLineOf = (root: FakeElement) => root.children[1]!
const countsLineOf = (root: FakeElement) => root.children[2]!

const connected = (runtimeId = '4f2a9c'): ConnectionState => ({ state: 'connected', runtimeId })

describe('createDiffHud', () => {
  it('renders the three HUD lines with the expected text', () => {
    created = []
    const hud = createDiffHud(createFakeDocument())
    hud.apply({
      connection: connected(),
      branch: 'cubito/auth-retry',
      counts: { files: 3, added: 12, removed: 4 }
    })
    const root = rootOf(hud)
    expect(connectionLineOf(root).children[1]!.textContent).toBe('conectado · runtime 4f2a9c')
    expect(modeLineOf(root).textContent).toBe('cubito/auth-retry · diff')
    expect(countsLineOf(root).textContent).toBe('3 archivos · +12 −4 contra base')
  })

  it('substitutes files/added/removed counts on line 3', () => {
    created = []
    const hud = createDiffHud(createFakeDocument())
    hud.apply({ connection: connected(), branch: 'b', counts: { files: 0, added: 0, removed: 0 } })
    expect(countsLineOf(rootOf(hud)).textContent).toBe('0 archivos · +0 −0 contra base')
  })
  ...
  it('sets the connection dot color via a --cubito-* var, per connection state', () => {
    ...
    hud.apply({
      ...
      branch: 'b',
      counts: { files: 0, added: 0, removed: 0 }
    })
